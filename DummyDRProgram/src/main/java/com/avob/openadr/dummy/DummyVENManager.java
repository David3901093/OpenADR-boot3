...
public class DummyVENManager {
	...
	public void init() {
		...
		try {
			VenMarketContextDto findMarketContextByName = marketContextControllerApi
					.findMarketContextByName(DummyVTN20bControllerConfig.MARKET_CONTEXT);
			marketcontextId = findMarketContextByName.getId();
			LOGGER.warn(
			...
			if (e.getCode() != HttpStatus.SC_NOT_FOUND) {
				...
				try {
					VenMarketContextDto createMarketContext = marketContextControllerApi
							.createMarketContext(dto);
					marketcontextId = createMarketContext.getId();
		...
		dummyVTN20bControllerConfig.getControlledVenCertificates().forEach(crt -> {
			...
			try {
				VenDto findVenByUsername = venControllerApi.findVenByUsername(oadr20bFingerprint);
				LOGGER.warn("Ven certificate: " + crt + " is already provisioned");
				if (findVenByUsername.getRegistrationId() != null) {
					oadr20bVenControllerApi.requestRegisterReport(oadr20bFingerprint);
				}
				return;
			} catch (ApiException e) {
				if (e.getCode() != org.apache.http.HttpStatus.SC_NOT_FOUND) {
					...
					try {
						...
						venControllerApi.createVen(dto);
	...
	@RabbitListener(
			bindings = @QueueBinding(
					value = @Queue(value = DummyVTN20bControllerConfig.OADR_APP_NOTIFICATION_REGISTER_REPORT_TOPIC, durable = "true"),
					exchange = @Exchange(value = "oadr.notification.exchange", type = "topic"),
					key = DummyVTN20bControllerConfig.OADR_APP_NOTIFICATION_REGISTER_REPORT_TOPIC
			)
	)
	public void onRegisterReportMessage(@Payload String message) {
		VenReportDto fromJson = gson.fromJson(message, VenReportDto.class);
		subscribe(fromJson);
	...
	@RabbitListener(
			bindings = @QueueBinding(
					value = @Queue(value = DummyVTN20bControllerConfig.OADR_APP_NOTIFICATION_UPDATE_REPORT_TOPIC_FLOAT, durable = "true"),
					exchange = @Exchange(value = "oadr.notification.exchange", type = "topic"),
					key = DummyVTN20bControllerConfig.OADR_APP_NOTIFICATION_UPDATE_REPORT_TOPIC_FLOAT
			)
	)
	public void onUpdateReportFloatMessage(@Payload String message) {
		List<OtherReportDataFloatDto> fromJson = gson.fromJson(message,
	...
	private void subscribe(VenReportDto venReport) {
		// 保持原有订阅逻辑不变
		List<OtherReportRequestDto> viewReportRequest;
		try {
			viewReportRequest = oadr20bVenControllerApi.viewReportRequest(venReport.getUsername(), null,
					null);
			viewReportRequest.forEach(request -> {
				try {
					oadr20bVenControllerApi.cancelReport(request.getReportRequestId(),
		...
		try {
			oadr20bVenControllerApi.subscribeOtherReportCapabilityDescriptionRid(subscriptions,
					venReport.getUsername());
		} catch (ApiException e) {
			LOGGER.error("Can't subcribe", e);
		}
