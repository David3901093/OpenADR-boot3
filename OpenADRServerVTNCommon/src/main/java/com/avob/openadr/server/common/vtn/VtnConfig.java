...
import com.avob.openadr.server.common.vtn.utils.PortUtil;
...
public class VtnConfig {
	...
	public static final String BROKER_USER_CONF = "spring.rabbitmq.user";
	public static final String BROKER_PASS_CONF = "spring.rabbitmq.password";
	public static final String BROKER_PORT_CONF = "spring.rabbitmq.port";
	public static final String BROKER_HOST_CONF = "spring.rabbitmq.host";
	...
	@PostConstruct
	public void init() {
		LOGGER.info("VTNConfig init …………");
		LOGGER.info("cert:"+this.getCert());
		...
		if (this.getCert() != null && this.getKey() != null) {
			...
			try {
				...
				LOGGER.info("VTN " + PRIVATE_KEY_CONF + " and " + CERTIFICATE_CONF + " given - HTTP SSL transport supported");
		...
		if (getBrokerPort() == null) {
			brokerPort = PortUtil.findAvailableTcpPort() ;
		}

		if (brokerSslPort == null) {
			brokerSslPort = PortUtil.findAvailableTcpPort() ;
		}
