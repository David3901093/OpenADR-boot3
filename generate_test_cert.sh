...
# MSYS_NO_PATHCONV=1 openssl s_client -key ven1.oadr.com.key -cert ven1.oadr.com.crt -CAfile oadr.com.crt -connect vtn.oadr.com:8181
...
gen_rsa_key_csr()
{
	...
	MSYS_NO_PATHCONV=1 openssl req -out $1.csr -newkey rsa:2048  -nodes -keyout $1.key \
		-subj "/C=$COUNTRY/ST=$STATE/L=$LOCALITY/O=$ORGANIZATION/OU=$ORGANIZATION/CN=$2" -reqexts req_ext -config $OPENSSL_CONFIG_FILENAME

	MSYS_NO_PATHCONV=1 openssl x509 -req -sha256 -days $5 -in $1.csr -CA $3.crt -CAkey $3.key -CAcreateserial -out $1.crt \
...
gen_ecc_key_csr()
{
	...
	MSYS_NO_PATHCONV=1 openssl req -out $1.csr -newkey ec:<(MSYS_NO_PATHCONV=1 openssl ecparam -name secp384r1) -nodes -keyout $1.key \
		-subj "/C=$COUNTRY/ST=$STATE/L=$LOCALITY/O=$ORGANIZATION/OU=$ORGANIZATION/CN=$2" \
		-addext "subjectAltName=DNS:$2,DNS:127.0.0.1,DNS:localhost"  
...
{
	echo "gen_pkcs12: $@"
	MSYS_NO_PATHCONV=1 openssl pkcs12 -export  -inkey $1.key -in $1.crt -certfile $CA_NAME.crt -out $1.p12 -password pass:changeme
}
...
gen_oadr20b_fingerprint()
{
	...
	MSYS_NO_PATHCONV=1 openssl x509 -in $1.crt -fingerprint -sha256 -noout | cut -d'=' -f2 | cat | tail -c 30 | sed 's#:##g' | tr '[:upper:]' '[:lower:]' > $1.fingerprint
}
# gen_oadr20a_fingerprint "in/out crt/fingerprint name"
gen_oadr20a_fingerprint()
{
	echo "gen_oadr20a_fingerprint: $@"
	MSYS_NO_PATHCONV=1 openssl x509 -in $1.crt -fingerprint -sha1 -noout | cut -d'=' -f2 | cat | tail -c 30 | sed 's#:##g' | tr '[:upper:]' '[:lower:]' > $1.fingerprint.oadr20a
}
# gen_selfsigned_key_crt "out key/crt name"
gen_selfsigned_key_crt()
{
	echo "gen_selfsigned_key_crt: $@"
	MSYS_NO_PATHCONV=1 openssl req -nodes -new -x509 -sha256 -keyout $1.key -out $1.crt \
